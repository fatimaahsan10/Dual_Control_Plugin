...
`config.control_method` (see schema.py):
  ...
  "ilqg" (default) -- compile the config into callables
      ...
      wizard-specific stand-in. Unchanged from before this dispatch
      existed.
...
Failure layers translated into plain language here:
  ...
  1b. ConstraintCompilationError (wizard/core_ilqr_adapter.py) -- same
     idea, for a constraint expression that failed to parse -- "ilqr"
     path only.
...
CONSTRAINT-WARNING CAPTURE ("ilqr" path only): validate_constraint_fn
...
constraining anything. This module wraps that one call in
`warnings.catch_warnings(record=True)` and surfaces any RuntimeWarning
from extensions/constraints/ as a plain-language ValidationReport message
-- advisory (does not fail validation on its own), since the constraint
math itself is not wrong, only inapplicable at the sampled points.
...
from wizard.core_ilqr_adapter import ConstraintCompilationError, compile_ilqr
...
def _validate_ilqr(config: ModelConfig) -> ValidationReport:
    ...
    # times for one genuinely-unenforced constraint row.
    ...
    return ValidationReport(ok=True, warnings=constraint_warnings)
