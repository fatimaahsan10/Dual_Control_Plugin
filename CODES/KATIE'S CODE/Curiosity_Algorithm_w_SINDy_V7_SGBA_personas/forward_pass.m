...
%Note: in Todorov code, there are no dt's in these calculations, and no q0
%term
...
%xu_f_dyn  = @(xau) (dynamics(dt, xau(ixa,:), xau(iu,:), constants, zeros(nw, 1), false, augment_states, 0) - xau(ixa,:))/dt;
...
A = fx; % Shown like this in Todorov 2007
B = fu;

% A = eye(size(fx, 1)) + fx*dt; % Shown like this in Todorov 2005
% B = fu*dt;
...
F = gx; % Shown without dt in Todorov 2007
E = gu;
...
q0 = dt*q0; % Shown with dt in both Todorov 2005 and 2007
q = dt*q;
Q = dt*Q;
r = dt*r;
R = dt*R;
