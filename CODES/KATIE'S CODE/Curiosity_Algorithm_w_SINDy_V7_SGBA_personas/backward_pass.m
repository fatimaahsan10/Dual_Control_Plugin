...
for k = N-1:-1:1
    ...
    if ~isempty(fxu)
%         fxuVx = vectens(s(:,k+1), fxu(:,:,:,k)); % iLQR code
%         G   = G + fxuVx;

        fxuVx = vectens(sx(:,k+1) + sxh(:,k+1), fxu(:,:,:,k)); % not 100% sure if this is correct
        Gx   = Gx + fxuVx;
    end
    
    %H = R(:,:,k)   + B(:,:,k)'*S(:,:,k+1)*B(:,:,k) + CuSCu;
    H = R(:,:,k) + B(:,:,k)'*(Sx(:,:,k+1) + Sxh(:,:,k+1) + 2*Sxxh(:,:,k+1))*B(:,:,k) + Cu_Sx_Cu + Du_K_Sxh_K_Du; % EQN (23) in Todorov 2007
    if ~isempty(fuu)
%         fuuVx = vectens(s(:,k+1), fuu(:,:,:,k)); % iLQR code
%         H   = H + fuuVx;

        fuuVx = vectens(sx(:,k+1) + sxh(:,k+1), fuu(:,:,:,k)); % not 100% sure if this is correct
        H   = H + fuuVx;
    end
    ...
    %H_reg = H + lambda*eye(m)*(regType == 1) + (lambda - min(eig(H)))*eye(m)*(regType == 3); 
    %option 3 (EQN (48) in Todorov 2007) for lambda reg, but no R in H from EQN (48) in Todorov 2007... 
    % NOTE: had min(eig(H - R(:, :, :)) here as EQN (48) doesn't have R, but I think that was a typo on Todorov's part 
    % Correction: min(eig(H - R(:, :, :)) performs much better on the car example
    
    if ~isempty(fuu)
        H_reg = H_reg + fuuVx;
    end
    
%     if ~issymmetric(H_reg)
%          warning('Hessian not symmetric, correcting')
%          H_reg = (H_reg+H_reg')*0.5;
%     end
...
%         L_i = lL(:,2:n+1);
% 
%         l_i = (lims(:, 2) - lims(:, 1))/2.*tanh(l_i + u(:,k)) + (lims(:, 2) + lims(:, 1))/2 - u(:,k);
%         
%         for control = 1:size(lims, 1)
%             L_i(l_i(control) + u(control,k) == lims(control, 1)) = 0;
%             L_i(l_i(control) + u(control,k) == lims(control, 2)) = 0;
%         end
...
%         [wsout,fval,result,output] = quadprog(H_reg,g,[],[],[],[],lower,upper,ws); % matlab equlivalent
%         l_i = wsout.X;
%         if result == 0
%             result = 5; % only for matlab, as result = 0 is fine, indiciating maxIterations reached
%         end
    ...
    %s0_alpha = s0_alpha + [l_i'*g, 0.5*l_i'*H*l_i+ 0.5*(c_Sx_c + d_K_Sxh_K_d)]; % AM modified, Jan 11, 2022, not sure if correct
    %s0_alpha = s0_alpha + [l_i'*g + 0.5*(c_Sx_c + d_K_Sxh_K_d), 0.5*l_i'*H*l_i];

    %s0(k)     = q0(k)      + s0(k+1)                        + 0.5*l_i'*H*l_i    + l_i'*g                + 0.5*cSc;  % EQN (42) in Todorov 2007, k terms in Todorov's code were a vector, and q0 term negliected
    %s(:,k)    = q(:,k)     + A(:,:,k)'*s(:,k+1)             + L_i'*H*l_i        + L_i'*g   + G'*l_i     + CxSc; % EQN (41) in Todorov 2007
    %S(:,:,k)  = Q(:,:,k)   + A(:,:,k)'*S(:,:,k+1)*A(:,:,k)  + L_i'*H*L_i        + L_i'*G   + G'*L_i     + CxSCx; % EQN (40) in Todorov 2007
    ...
    if ~isempty(fxx)
%         S(:,:,k) = S(:,:,k) + vectens(s(:,k+1),fxx(:,:,:,k)); % iLQR code
        Sx(:,:,k) = Sx(:,:,k) + vectens(sx(:,k+1) + sxh(:,k+1), fxx(:,:,:,k)); % not 100% sure if this is correct
    end
